 ...
 * `Compy_RtpHeader_size(self)`.
 *
 * @return The pointer to a passed buffer.
 */
uint8_t *Compy_RtpHeader_serialize(
    Compy_RtpHeader self, uint8_t buffer[restrict]) COMPY_PRIV_MUST_USE;

/**
 * Parses an RTP header from a binary buffer.
 *
 * Fields in @p self will be in network byte order, matching the convention
 * used by #Compy_RtpHeader_serialize.
 *
 * @param[out] self The parsed RTP header.
 * @param[in] data The buffer to parse from.
 * @param[in] len The length of @p data in bytes.
 *
 * @pre `self != NULL`
 * @pre `data != NULL`
 *
 * @return The total header size in bytes on success, or -1 if @p len is too
 * small or the version field is not 2.
 */
int Compy_RtpHeader_deserialize(
    Compy_RtpHeader *restrict self, const uint8_t *data,
