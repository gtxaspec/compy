...
Compy_Request Compy_Request_uninit(void) {
    Compy_Request self;
...
Compy_ParseResult
Compy_Request_parse(Compy_Request *restrict self, CharSlice99 input) {
    assert(self);
    ...
    MATCH(Compy_RequestLine_parse(&self->start_line, input));
    MATCH(Compy_HeaderMap_parse(&self->header_map, input));

    CharSlice99 content_length;
    size_t content_length_int = 0;
    const bool content_length_found = Compy_HeaderMap_find(
...
bool Compy_Request_eq(
    ...
    return Compy_RequestLine_eq(&lhs->start_line, &rhs->start_line) &&
