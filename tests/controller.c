...
Client_options(VSelf, Compy_Context *ctx, const Compy_Request *req) {
    ...
    compy_header(ctx, HEADER_TEST_METHOD, "options");
...
Client_describe(VSelf, Compy_Context *ctx, const Compy_Request *req) {
    ...
    compy_header(ctx, HEADER_TEST_METHOD, "describe");
    const ssize_t ret = compy_respond_ok(ctx);
    assert(ret > 0);
}
...
Client_setup(VSelf, Compy_Context *ctx, const Compy_Request *req) {
    ...
    compy_header(ctx, HEADER_TEST_METHOD, "setup");
    const ssize_t ret = compy_respond_ok(ctx);
    assert(ret > 0);
}
...
Client_play(VSelf, Compy_Context *ctx, const Compy_Request *req) {
    ...
    assert(req == &play_req);
    compy_header(ctx, HEADER_TEST_METHOD, "play");
    const ssize_t ret = compy_respond_ok(ctx);
    assert(ret > 0);
}
...
Client_teardown(VSelf, Compy_Context *ctx, const Compy_Request *req) {
    ...
    compy_header(ctx, HEADER_TEST_METHOD, "teardown");
    const ssize_t ret = compy_respond_ok(ctx);
    assert(ret > 0);
}
...
Client_unknown(VSelf, Compy_Context *ctx, const Compy_Request *req) {
    ...
    compy_header(ctx, HEADER_TEST_METHOD, "abracadabra");
    const ssize_t ret = compy_respond_ok(ctx);
    assert(ret > 0);
}
...
Client_before(VSelf, Compy_Context *ctx, const Compy_Request *req) {
    ...
    compy_header(ctx, COMPY_HEADER_AUTHORIZATION, "WWW-Authenticate");
    compy_header(ctx, COMPY_HEADER_SERVER, "MyServer");
