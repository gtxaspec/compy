...
TEST context_creation(void) {
    char buffer[32] = {0};
    const Compy_Writer w = compy_string_writer(buffer);

    const uint32_t cseq = 123;

    Compy_Context *ctx = Compy_Context_new(w, cseq);

    const Compy_Writer got_writer = Compy_Context_get_writer(ctx);
    ASSERT_EQ(w.self, got_writer.self);
    ASSERT_EQ(w.vptr, got_writer.vptr);

    ASSERT_EQ(cseq, Compy_Context_get_cseq(ctx));
    ASSERT_EQ(0, Compy_Context_get_ret(ctx));
...
TEST respond_empty(void) {
    ...
    ssize_t ret = compy_respond(ctx, COMPY_STATUS_NOT_FOUND, "Not found");
    ASSERT_EQ((ssize_t)strlen(expected), ret);
    ASSERT_EQ(ret, Compy_Context_get_ret(ctx));
...
TEST respond_ok(void) {
    ...
    ssize_t ret = compy_respond_ok(ctx);
    ASSERT_EQ((ssize_t)strlen(expected), ret);
    ASSERT_EQ(ret, Compy_Context_get_ret(ctx));
    ASSERT_STR_EQ(expected, buffer);
...
TEST respond_internal_error(void) {
    ...
    ssize_t ret = compy_respond_internal_error(ctx);
    ASSERT_EQ((ssize_t)strlen(expected), ret);
    ASSERT_EQ(ret, Compy_Context_get_ret(ctx));
    ASSERT_STR_EQ(expected, buffer);
